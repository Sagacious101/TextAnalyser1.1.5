...
def path_to_file():
    ...
    try:
        ...
        text_analyser = TextAnalyser.TextAnalyser(source_file=packs['file_name'][0]['text'], parts_of_speech=grammenes, wc_width=int(packs['width_entry'][0].get()), wc_height=int(packs['height_entry'][0].get()))
...
packs['path_to_file_lable'] = [tk.Button(text='Выберете файл с текстом', command=select_file, font=("Arial", 12)), (len(packs), 0)]
...
packs['color_canvas'] = [tk.Canvas(width=30, height=30, bg='gray'), (len(packs), 0)]
packs['change_color_buttom'] = [tk.Button(text='Выбрать цвет фона', command=change_color), (len(packs), 0)]

packs['path_to_file_buttom'] = [tk.Button(text='Провести анализ текста', font=("Arial", 14), command=path_to_file), (len(packs), 0)]
